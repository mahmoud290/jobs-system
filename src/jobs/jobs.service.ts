...
export class JobsService {
  ...
  async shortlistApplication(jobId: number, userId: number) {
    ...
    await this.mailerService.sendShortlistNotification({ to: user.email, jobTitle: job.title });
  ...
  async applyToJob(jobId: number, userId: number) {
    ...
    await this.mailerService.sendApplicationEmail({ to: user.email, jobTitle: job.title });
  ...
  for (const user of job.appliedUsers) {
    await this.notificationsService.sendNotification(
      user,
  ...
  return { message: 'Job closed and notifications sent' };
}
}
